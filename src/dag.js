// The below is @cjongseok's dag.js libary
// https://github.com/cjongseok/dag.js

// Having a difficult time getting the imports right, so copy pasted
// the library here for now.
...
class Dag {
  ...
  get T() {
    return Object.keys(this.tagObjs).reduce(
      (previous, tag) => previous.concat([tag]),
      []
    );
  }
  ...
  get size() {
    return Object.keys(this.edges).reduce(
      (previous, key) => previous + this.edges[key].length,
      0
    );
  }
  /*
    ...
    if (this.edges[to] !== undefined) {
      ...
      if (edge !== undefined) {
        return {
          ...
          weight: edge.w,
          tags: edge.ts.reduce(
            (previous, tagObject) => previous.concat([tagObject.name]),
            []
          )
        };
      }
  ...
  add(from, to, tags, weight) {
    ...
    const edge = {
      ...
      w: weight,
      ts: tagArray.reduce(
        (previous, tag) => previous.concat([this.tagObjs[tag]]),
        []
      )
    };
  ...
  deepClone() {
    ...
    Object.keys(this.edges).forEach(to => {
      this.edges[to].forEach(e => {
        // clone the edge
        const tags = e.ts.reduce(
          (previous, tagObj) => previous.concat([tagObj.name]),
          []
        );
  ...
  edgesFrom(from) {
    ...
    Object.keys(this.edges).forEach(key => {
      this.edges[key].forEach(e => {
        if (e.f === from) {
          const cloned = { from: e.f, to: key, weight: e.w };
          cloned.tags = e.ts.reduce(
            (previous, tagObject) => previous.concat([tagObject.name]),
            []
          );
  ...
  edgesTo(to) {
    ...
    const dag = new Dag();
    this.edges[to].forEach(e => {
      const cloned = { from: e.f, to, weight: e.w };
      cloned.tags = e.ts.reduce(
        (previous, tagObject) => previous.concat([tagObject.name]),
        []
      );
   ...
   *                          empty DAG, if the vertex does not exist or there is no edges around
   *                          it.
   */
  neighbourhood(vertex) {
    ...
    this.edges[vertex].forEach(e => {
      const cloned = { from: e.f, to: vertex, weight: e.w };
      cloned.tags = e.ts.reduce(
        (previous, tagObject) => previous.concat([tagObject.name]),
        []
      );
  ...
  tagOrder(tag) {
    ...
    const verticies = Object.keys(this.tagInvertedIndex[tag]).reduce(
      (previous, to) => {
  ...
  tagSize(tag) {
    ...
    return Object.keys(this.tagInvertedIndex[tag]).reduce(
      (count, to) => count + this.tagInvertedIndex[tag][to].length,
      0
    );
  }
  ...
  tagKind(tag) {
    ...
    const tags = Object.keys(this.tagInvertedIndex[tag]).reduce(
      (previous, to) => {
        if (this.tagInvertedIndex[tag][to].length > 0) {
          this.tagInvertedIndex[tag][to].forEach(e => {
            e.ts.forEach(tagObj => {
   ...
   * @returns {array}     A deep-cloned sub-DAG of itself, if there is an edge including 'tag'.
   *                      Undefined, otherwise.
   */
  filterByTag(tag) {
    if (!(tag in this.tagInvertedIndex)) {
    ...
    Object.keys(filteredEdges).forEach(to => {
      filteredEdges[to].forEach(edge => {
        const cloned = { from: edge.f, to, tags: [], weight: undefined };
        cloned.tags = edge.ts.reduce(
          (previous, tagObj) => previous.concat([tagObj.name]),
          []
        );
  ...
  removeEdge(from, to) {
    ...
    removed.ts.forEach(tagObj => {
      this.tagInvertedIndex[tagObj.name][to] = this.tagInvertedIndex[
        tagObj.name
      ][to].filter(edge => edge.f !== from);
  ...
  removeVertex(vertex) {
    ...
    Object.keys(this.edges).forEach(to => {
      ...
      tagsToRemove.forEach(tag => {
        this.tagInvertedIndex[tag][to] = this.tagInvertedIndex[tag][to].filter(
          e => e.f !== vertex
        );
        if (this.tagInvertedIndex[tag][to].length === 0) {
